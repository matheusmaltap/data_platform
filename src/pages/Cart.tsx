    ...
    return (
      <div className="p-8 max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('cart.successTitle')}</h1>
          <p className="text-sm text-gray-500 mb-2">
            {t('cart.successMessage')}
          </p>
          <p className="text-sm text-gray-400 mb-8" dangerouslySetInnerHTML={{ __html: t('cart.successDetail') }} />
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => navigate('/')}
              className="px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
              {t('cart.backToDiscovery')}
            </button>
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <button onClick={() => navigate('/')} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6">
        <ChevronLeft size={16} /> {t('cart.backToDiscovery')}
      </button>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{t('cart.selectedCount', { count })}</p>
        </div>
      </div>
      ...
      {cartProducts.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">{t('cart.empty')}</p>
          <button onClick={() => navigate('/')}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 bg-gray-50">
              <p className="text-sm font-semibold text-gray-700">{t('cart.selectedProducts')}</p>
            </div>
            <div className="divide-y divide-gray-100">
              {cartProducts.map(({ productId, product }) => (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <label className="text-sm font-semibold text-gray-700">
                {t('cart.justification')} <span className="text-red-500">*</span>
              </label>
            </div>
            <textarea
              value={justification}
              ...
              rows={3}
              placeholder={t('cart.justificationPlaceholder')}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-xs text-gray-400">
              {t('cart.justificationHint')}
            </p>
          </div>

          {/* Approval pipeline info */}
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-blue-800 mb-3">{t('cart.approvalFlow')}</h3>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-blue-700">{t('cart.step1')}</span>
              </div>
